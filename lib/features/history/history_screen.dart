...
class _RangeSelector extends StatelessWidget {
  ...
  final ValueChanged<HistoryRange>
  onSelected;
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        children: HistoryRange.values.map(
              (range) {
            final isSelected =
                selected == range;
...
class _MetricSelector extends StatelessWidget {
  ...
  final ValueChanged<HistoryMetric>
  onSelected;
  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(
      ...
      const BouncingScrollPhysics(),
      child: Row(
        children: HistoryMetric.values.map(
              (metric) {
            final isSelected =
                metric == selected;
...
class _SensorChartCard
  ...
  Widget build(BuildContext context) {
    final accent =
    _metricColor(history.selectedMetric);
  ...
  LineChartData _buildChartData(
    ...
    final rawMinimum = values.reduce(
          (a, b) => a < b ? a : b,
    );

    final rawMaximum = values.reduce(
          (a, b) => a > b ? a : b,
    );
    ...
    final verticalInterval = math.max(
      1.0,
      (maximumY - minimumY) / 4,
    );
    ...
    return GridView.count(
      ...
      children: [
        ...
        _StatisticCard(
          label: 'Highest',
          value:
          '${_formatValue(history.maximumValue)} ${history.metricUnit}',
          icon:
        ...
        _StatisticCard(
          label: 'Lowest',
          value:
          '${_formatValue(history.minimumValue)} ${history.metricUnit}',
          icon:
